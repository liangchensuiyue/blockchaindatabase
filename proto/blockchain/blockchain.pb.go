// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.25.0
// 	protoc        v3.13.0
// source: blockchain.proto

package blockchaingrpc

import (
	context "context"
	proto "github.com/golang/protobuf/proto"
	grpc "google.golang.org/grpc"
	codes "google.golang.org/grpc/codes"
	status "google.golang.org/grpc/status"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// This is a compile-time assertion that a sufficiently up-to-date version
// of the legacy proto package is being used.
const _ = proto.ProtoPackageIsVersion4

type ShareChanName struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
}

func (x *ShareChanName) Reset() {
	*x = ShareChanName{}
	if protoimpl.UnsafeEnabled {
		mi := &file_blockchain_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ShareChanName) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ShareChanName) ProtoMessage() {}

func (x *ShareChanName) ProtoReflect() protoreflect.Message {
	mi := &file_blockchain_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ShareChanName.ProtoReflect.Descriptor instead.
func (*ShareChanName) Descriptor() ([]byte, []int) {
	return file_blockchain_proto_rawDescGZIP(), []int{0}
}

func (x *ShareChanName) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type ShareChanBody struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Users          []string `protobuf:"bytes,1,rep,name=users,proto3" json:"users,omitempty"`
	Key            []byte   `protobuf:"bytes,2,opt,name=Key,proto3" json:"Key,omitempty"`
	JoinKey        []byte   `protobuf:"bytes,3,opt,name=JoinKey,proto3" json:"JoinKey,omitempty"`
	Creator        string   `protobuf:"bytes,4,opt,name=Creator,proto3" json:"Creator,omitempty"`
	CreatorAddress string   `protobuf:"bytes,5,opt,name=CreatorAddress,proto3" json:"CreatorAddress,omitempty"`
	Channame       string   `protobuf:"bytes,6,opt,name=Channame,proto3" json:"Channame,omitempty"`
}

func (x *ShareChanBody) Reset() {
	*x = ShareChanBody{}
	if protoimpl.UnsafeEnabled {
		mi := &file_blockchain_proto_msgTypes[1]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ShareChanBody) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ShareChanBody) ProtoMessage() {}

func (x *ShareChanBody) ProtoReflect() protoreflect.Message {
	mi := &file_blockchain_proto_msgTypes[1]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ShareChanBody.ProtoReflect.Descriptor instead.
func (*ShareChanBody) Descriptor() ([]byte, []int) {
	return file_blockchain_proto_rawDescGZIP(), []int{1}
}

func (x *ShareChanBody) GetUsers() []string {
	if x != nil {
		return x.Users
	}
	return nil
}

func (x *ShareChanBody) GetKey() []byte {
	if x != nil {
		return x.Key
	}
	return nil
}

func (x *ShareChanBody) GetJoinKey() []byte {
	if x != nil {
		return x.JoinKey
	}
	return nil
}

func (x *ShareChanBody) GetCreator() string {
	if x != nil {
		return x.Creator
	}
	return ""
}

func (x *ShareChanBody) GetCreatorAddress() string {
	if x != nil {
		return x.CreatorAddress
	}
	return ""
}

func (x *ShareChanBody) GetChanname() string {
	if x != nil {
		return x.Channame
	}
	return ""
}

type Heartbeat struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	IsAccountant bool   `protobuf:"varint,1,opt,name=is_accountant,json=isAccountant,proto3" json:"is_accountant,omitempty"`
	BlockNums    int32  `protobuf:"varint,2,opt,name=block_nums,json=blockNums,proto3" json:"block_nums,omitempty"`
	LocalIp      string `protobuf:"bytes,3,opt,name=local_ip,json=localIp,proto3" json:"local_ip,omitempty"`
	LocalPort    int32  `protobuf:"varint,4,opt,name=local_port,json=localPort,proto3" json:"local_port,omitempty"`
}

func (x *Heartbeat) Reset() {
	*x = Heartbeat{}
	if protoimpl.UnsafeEnabled {
		mi := &file_blockchain_proto_msgTypes[2]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Heartbeat) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Heartbeat) ProtoMessage() {}

func (x *Heartbeat) ProtoReflect() protoreflect.Message {
	mi := &file_blockchain_proto_msgTypes[2]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Heartbeat.ProtoReflect.Descriptor instead.
func (*Heartbeat) Descriptor() ([]byte, []int) {
	return file_blockchain_proto_rawDescGZIP(), []int{2}
}

func (x *Heartbeat) GetIsAccountant() bool {
	if x != nil {
		return x.IsAccountant
	}
	return false
}

func (x *Heartbeat) GetBlockNums() int32 {
	if x != nil {
		return x.BlockNums
	}
	return 0
}

func (x *Heartbeat) GetLocalIp() string {
	if x != nil {
		return x.LocalIp
	}
	return ""
}

func (x *Heartbeat) GetLocalPort() int32 {
	if x != nil {
		return x.LocalPort
	}
	return 0
}

type NodeInfo struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Passworld string `protobuf:"bytes,1,opt,name=passworld,proto3" json:"passworld,omitempty"`
	LocalIp   string `protobuf:"bytes,2,opt,name=local_ip,json=localIp,proto3" json:"local_ip,omitempty"`
	LocalPort int32  `protobuf:"varint,3,opt,name=local_port,json=localPort,proto3" json:"local_port,omitempty"`
}

func (x *NodeInfo) Reset() {
	*x = NodeInfo{}
	if protoimpl.UnsafeEnabled {
		mi := &file_blockchain_proto_msgTypes[3]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *NodeInfo) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*NodeInfo) ProtoMessage() {}

func (x *NodeInfo) ProtoReflect() protoreflect.Message {
	mi := &file_blockchain_proto_msgTypes[3]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use NodeInfo.ProtoReflect.Descriptor instead.
func (*NodeInfo) Descriptor() ([]byte, []int) {
	return file_blockchain_proto_rawDescGZIP(), []int{3}
}

func (x *NodeInfo) GetPassworld() string {
	if x != nil {
		return x.Passworld
	}
	return ""
}

func (x *NodeInfo) GetLocalIp() string {
	if x != nil {
		return x.LocalIp
	}
	return ""
}

func (x *NodeInfo) GetLocalPort() int32 {
	if x != nil {
		return x.LocalPort
	}
	return 0
}

type RequestBody struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	UserAddress string       `protobuf:"bytes,1,opt,name=user_address,json=userAddress,proto3" json:"user_address,omitempty"`
	Strict      bool         `protobuf:"varint,2,opt,name=strict,proto3" json:"strict,omitempty"`
	Tx          *Transaction `protobuf:"bytes,3,opt,name=Tx,proto3" json:"Tx,omitempty"`
}

func (x *RequestBody) Reset() {
	*x = RequestBody{}
	if protoimpl.UnsafeEnabled {
		mi := &file_blockchain_proto_msgTypes[4]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *RequestBody) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RequestBody) ProtoMessage() {}

func (x *RequestBody) ProtoReflect() protoreflect.Message {
	mi := &file_blockchain_proto_msgTypes[4]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RequestBody.ProtoReflect.Descriptor instead.
func (*RequestBody) Descriptor() ([]byte, []int) {
	return file_blockchain_proto_rawDescGZIP(), []int{4}
}

func (x *RequestBody) GetUserAddress() string {
	if x != nil {
		return x.UserAddress
	}
	return ""
}

func (x *RequestBody) GetStrict() bool {
	if x != nil {
		return x.Strict
	}
	return false
}

func (x *RequestBody) GetTx() *Transaction {
	if x != nil {
		return x.Tx
	}
	return nil
}

type Nodes struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Nodes []*NodeInfo `protobuf:"bytes,1,rep,name=nodes,proto3" json:"nodes,omitempty"`
}

func (x *Nodes) Reset() {
	*x = Nodes{}
	if protoimpl.UnsafeEnabled {
		mi := &file_blockchain_proto_msgTypes[5]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Nodes) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Nodes) ProtoMessage() {}

func (x *Nodes) ProtoReflect() protoreflect.Message {
	mi := &file_blockchain_proto_msgTypes[5]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Nodes.ProtoReflect.Descriptor instead.
func (*Nodes) Descriptor() ([]byte, []int) {
	return file_blockchain_proto_rawDescGZIP(), []int{5}
}

func (x *Nodes) GetNodes() []*NodeInfo {
	if x != nil {
		return x.Nodes
	}
	return nil
}

type VerifyInfo struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Status bool   `protobuf:"varint,1,opt,name=status,proto3" json:"status,omitempty"`
	Info   string `protobuf:"bytes,2,opt,name=info,proto3" json:"info,omitempty"`
}

func (x *VerifyInfo) Reset() {
	*x = VerifyInfo{}
	if protoimpl.UnsafeEnabled {
		mi := &file_blockchain_proto_msgTypes[6]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *VerifyInfo) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*VerifyInfo) ProtoMessage() {}

func (x *VerifyInfo) ProtoReflect() protoreflect.Message {
	mi := &file_blockchain_proto_msgTypes[6]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use VerifyInfo.ProtoReflect.Descriptor instead.
func (*VerifyInfo) Descriptor() ([]byte, []int) {
	return file_blockchain_proto_rawDescGZIP(), []int{6}
}

func (x *VerifyInfo) GetStatus() bool {
	if x != nil {
		return x.Status
	}
	return false
}

func (x *VerifyInfo) GetInfo() string {
	if x != nil {
		return x.Info
	}
	return ""
}

type Transaction struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Key          string `protobuf:"bytes,1,opt,name=key,proto3" json:"key,omitempty"`
	Value        []byte `protobuf:"bytes,2,opt,name=value,proto3" json:"value,omitempty"` // []byte
	DataType     int32  `protobuf:"varint,3,opt,name=data_type,json=dataType,proto3" json:"data_type,omitempty"`
	Timestamp    uint64 `protobuf:"varint,4,opt,name=timestamp,proto3" json:"timestamp,omitempty"`                            // 时间错
	PublicKey    []byte `protobuf:"bytes,5,opt,name=public_key,json=publicKey,proto3" json:"public_key,omitempty"`            // 用户公钥， 可以有多个(用户共享数据)
	Hash         []byte `protobuf:"bytes,6,opt,name=hash,proto3" json:"hash,omitempty"`                                       // 交易 hash
	PreBlockHash []byte `protobuf:"bytes,7,opt,name=pre_block_hash,json=preBlockHash,proto3" json:"pre_block_hash,omitempty"` // 在链中，用户相同交易所在前一个区块hahs(可以有多个，如果该数据类型是共享的话)
	Signature    []byte `protobuf:"bytes,8,opt,name=signature,proto3" json:"signature,omitempty"`                             // 用户对交易的签名
	ShareChan    string `protobuf:"bytes,9,opt,name=ShareChan,proto3" json:"ShareChan,omitempty"`
	Share        bool   `protobuf:"varint,10,opt,name=share,proto3" json:"share,omitempty"`
}

func (x *Transaction) Reset() {
	*x = Transaction{}
	if protoimpl.UnsafeEnabled {
		mi := &file_blockchain_proto_msgTypes[7]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Transaction) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Transaction) ProtoMessage() {}

func (x *Transaction) ProtoReflect() protoreflect.Message {
	mi := &file_blockchain_proto_msgTypes[7]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Transaction.ProtoReflect.Descriptor instead.
func (*Transaction) Descriptor() ([]byte, []int) {
	return file_blockchain_proto_rawDescGZIP(), []int{7}
}

func (x *Transaction) GetKey() string {
	if x != nil {
		return x.Key
	}
	return ""
}

func (x *Transaction) GetValue() []byte {
	if x != nil {
		return x.Value
	}
	return nil
}

func (x *Transaction) GetDataType() int32 {
	if x != nil {
		return x.DataType
	}
	return 0
}

func (x *Transaction) GetTimestamp() uint64 {
	if x != nil {
		return x.Timestamp
	}
	return 0
}

func (x *Transaction) GetPublicKey() []byte {
	if x != nil {
		return x.PublicKey
	}
	return nil
}

func (x *Transaction) GetHash() []byte {
	if x != nil {
		return x.Hash
	}
	return nil
}

func (x *Transaction) GetPreBlockHash() []byte {
	if x != nil {
		return x.PreBlockHash
	}
	return nil
}

func (x *Transaction) GetSignature() []byte {
	if x != nil {
		return x.Signature
	}
	return nil
}

func (x *Transaction) GetShareChan() string {
	if x != nil {
		return x.ShareChan
	}
	return ""
}

func (x *Transaction) GetShare() bool {
	if x != nil {
		return x.Share
	}
	return false
}

type Block struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Hash         []byte         `protobuf:"bytes,1,opt,name=hash,proto3" json:"hash,omitempty"`                                       // 块 hash
	BlockId      uint64         `protobuf:"varint,2,opt,name=block_id,json=blockId,proto3" json:"block_id,omitempty"`                 // 块序号
	Timestamp    uint64         `protobuf:"varint,3,opt,name=timestamp,proto3" json:"timestamp,omitempty"`                            // 时间戳
	PreBlockHash []byte         `protobuf:"bytes,4,opt,name=pre_block_hash,json=preBlockHash,proto3" json:"pre_block_hash,omitempty"` // 区块链中前一个区块hash
	MerkelRoot   []byte         `protobuf:"bytes,5,opt,name=MerkelRoot,proto3" json:"MerkelRoot,omitempty"`                           // 区块链中前一个区块hash
	TxInfos      []*Transaction `protobuf:"bytes,6,rep,name=TxInfos,proto3" json:"TxInfos,omitempty"`                                 // 所要打包的交易
	Signature    []byte         `protobuf:"bytes,7,opt,name=signature,proto3" json:"signature,omitempty"`
}

func (x *Block) Reset() {
	*x = Block{}
	if protoimpl.UnsafeEnabled {
		mi := &file_blockchain_proto_msgTypes[8]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Block) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Block) ProtoMessage() {}

func (x *Block) ProtoReflect() protoreflect.Message {
	mi := &file_blockchain_proto_msgTypes[8]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Block.ProtoReflect.Descriptor instead.
func (*Block) Descriptor() ([]byte, []int) {
	return file_blockchain_proto_rawDescGZIP(), []int{8}
}

func (x *Block) GetHash() []byte {
	if x != nil {
		return x.Hash
	}
	return nil
}

func (x *Block) GetBlockId() uint64 {
	if x != nil {
		return x.BlockId
	}
	return 0
}

func (x *Block) GetTimestamp() uint64 {
	if x != nil {
		return x.Timestamp
	}
	return 0
}

func (x *Block) GetPreBlockHash() []byte {
	if x != nil {
		return x.PreBlockHash
	}
	return nil
}

func (x *Block) GetMerkelRoot() []byte {
	if x != nil {
		return x.MerkelRoot
	}
	return nil
}

func (x *Block) GetTxInfos() []*Transaction {
	if x != nil {
		return x.TxInfos
	}
	return nil
}

func (x *Block) GetSignature() []byte {
	if x != nil {
		return x.Signature
	}
	return nil
}

// 区块同步
type ReqBlock struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Hash    []byte `protobuf:"bytes,1,opt,name=hash,proto3" json:"hash,omitempty"`                       // 本地节点最新区块hash
	BlockId uint64 `protobuf:"varint,2,opt,name=block_id,json=blockId,proto3" json:"block_id,omitempty"` // 最新区块序号
}

func (x *ReqBlock) Reset() {
	*x = ReqBlock{}
	if protoimpl.UnsafeEnabled {
		mi := &file_blockchain_proto_msgTypes[9]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ReqBlock) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ReqBlock) ProtoMessage() {}

func (x *ReqBlock) ProtoReflect() protoreflect.Message {
	mi := &file_blockchain_proto_msgTypes[9]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ReqBlock.ProtoReflect.Descriptor instead.
func (*ReqBlock) Descriptor() ([]byte, []int) {
	return file_blockchain_proto_rawDescGZIP(), []int{9}
}

func (x *ReqBlock) GetHash() []byte {
	if x != nil {
		return x.Hash
	}
	return nil
}

func (x *ReqBlock) GetBlockId() uint64 {
	if x != nil {
		return x.BlockId
	}
	return 0
}

type ResBlocks struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// bytes latest_block_hash = 1; // 同步过来的最新区块hash
	Blocks []*Block `protobuf:"bytes,1,rep,name=blocks,proto3" json:"blocks,omitempty"` // 同步过来的区块数据
}

func (x *ResBlocks) Reset() {
	*x = ResBlocks{}
	if protoimpl.UnsafeEnabled {
		mi := &file_blockchain_proto_msgTypes[10]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ResBlocks) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ResBlocks) ProtoMessage() {}

func (x *ResBlocks) ProtoReflect() protoreflect.Message {
	mi := &file_blockchain_proto_msgTypes[10]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ResBlocks.ProtoReflect.Descriptor instead.
func (*ResBlocks) Descriptor() ([]byte, []int) {
	return file_blockchain_proto_rawDescGZIP(), []int{10}
}

func (x *ResBlocks) GetBlocks() []*Block {
	if x != nil {
		return x.Blocks
	}
	return nil
}

type QueryInfo struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Key               string `protobuf:"bytes,1,opt,name=key,proto3" json:"key,omitempty"`                                                          // 查询关键字
	UserPublicKeyHash []byte `protobuf:"bytes,2,opt,name=User_public_key_hash,json=UserPublicKeyHash,proto3" json:"User_public_key_hash,omitempty"` // 用户公钥hash；也就是地址
}

func (x *QueryInfo) Reset() {
	*x = QueryInfo{}
	if protoimpl.UnsafeEnabled {
		mi := &file_blockchain_proto_msgTypes[11]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *QueryInfo) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*QueryInfo) ProtoMessage() {}

func (x *QueryInfo) ProtoReflect() protoreflect.Message {
	mi := &file_blockchain_proto_msgTypes[11]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use QueryInfo.ProtoReflect.Descriptor instead.
func (*QueryInfo) Descriptor() ([]byte, []int) {
	return file_blockchain_proto_rawDescGZIP(), []int{11}
}

func (x *QueryInfo) GetKey() string {
	if x != nil {
		return x.Key
	}
	return ""
}

func (x *QueryInfo) GetUserPublicKeyHash() []byte {
	if x != nil {
		return x.UserPublicKeyHash
	}
	return nil
}

type ResPackBlock struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Code bool `protobuf:"varint,1,opt,name=code,proto3" json:"code,omitempty"` // 区块校验是否成功
}

func (x *ResPackBlock) Reset() {
	*x = ResPackBlock{}
	if protoimpl.UnsafeEnabled {
		mi := &file_blockchain_proto_msgTypes[12]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ResPackBlock) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ResPackBlock) ProtoMessage() {}

func (x *ResPackBlock) ProtoReflect() protoreflect.Message {
	mi := &file_blockchain_proto_msgTypes[12]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ResPackBlock.ProtoReflect.Descriptor instead.
func (*ResPackBlock) Descriptor() ([]byte, []int) {
	return file_blockchain_proto_rawDescGZIP(), []int{12}
}

func (x *ResPackBlock) GetCode() bool {
	if x != nil {
		return x.Code
	}
	return false
}

type PackBlock struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Hash []byte `protobuf:"bytes,1,opt,name=hash,proto3" json:"hash,omitempty"` // 区块hash
}

func (x *PackBlock) Reset() {
	*x = PackBlock{}
	if protoimpl.UnsafeEnabled {
		mi := &file_blockchain_proto_msgTypes[13]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *PackBlock) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PackBlock) ProtoMessage() {}

func (x *PackBlock) ProtoReflect() protoreflect.Message {
	mi := &file_blockchain_proto_msgTypes[13]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PackBlock.ProtoReflect.Descriptor instead.
func (*PackBlock) Descriptor() ([]byte, []int) {
	return file_blockchain_proto_rawDescGZIP(), []int{13}
}

func (x *PackBlock) GetHash() []byte {
	if x != nil {
		return x.Hash
	}
	return nil
}

type ResQuery struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Status bool   `protobuf:"varint,1,opt,name=status,proto3" json:"status,omitempty"` // 查找状态
	Data   []byte `protobuf:"bytes,2,opt,name=data,proto3" json:"data,omitempty"`      // 数据(如果找到的)
}

func (x *ResQuery) Reset() {
	*x = ResQuery{}
	if protoimpl.UnsafeEnabled {
		mi := &file_blockchain_proto_msgTypes[14]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ResQuery) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ResQuery) ProtoMessage() {}

func (x *ResQuery) ProtoReflect() protoreflect.Message {
	mi := &file_blockchain_proto_msgTypes[14]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ResQuery.ProtoReflect.Descriptor instead.
func (*ResQuery) Descriptor() ([]byte, []int) {
	return file_blockchain_proto_rawDescGZIP(), []int{14}
}

func (x *ResQuery) GetStatus() bool {
	if x != nil {
		return x.Status
	}
	return false
}

func (x *ResQuery) GetData() []byte {
	if x != nil {
		return x.Data
	}
	return nil
}

var File_blockchain_proto protoreflect.FileDescriptor

var file_blockchain_proto_rawDesc = []byte{
	,
}

var (
	file_blockchain_proto_rawDescOnce sync.Once
	file_blockchain_proto_rawDescData = file_blockchain_proto_rawDesc
)

func file_blockchain_proto_rawDescGZIP() []byte {
	file_blockchain_proto_rawDescOnce.Do(func() {
		file_blockchain_proto_rawDescData = protoimpl.X.CompressGZIP(file_blockchain_proto_rawDescData)
	})
	return file_blockchain_proto_rawDescData
}

var file_blockchain_proto_msgTypes = make([]protoimpl.MessageInfo, 15)
var file_blockchain_proto_goTypes = []interface{}{
	(*ShareChanName)(nil), // 0: blockchaingrpc.ShareChanName
	(*ShareChanBody)(nil), // 1: blockchaingrpc.ShareChanBody
	(*Heartbeat)(nil),     // 2: blockchaingrpc.Heartbeat
	(*NodeInfo)(nil),      // 3: blockchaingrpc.NodeInfo
	(*RequestBody)(nil),   // 4: blockchaingrpc.RequestBody
	(*Nodes)(nil),         // 5: blockchaingrpc.Nodes
	(*VerifyInfo)(nil),    // 6: blockchaingrpc.VerifyInfo
	(*Transaction)(nil),   // 7: blockchaingrpc.Transaction
	(*Block)(nil),         // 8: blockchaingrpc.Block
	(*ReqBlock)(nil),      // 9: blockchaingrpc.ReqBlock
	(*ResBlocks)(nil),     // 10: blockchaingrpc.ResBlocks
	(*QueryInfo)(nil),     // 11: blockchaingrpc.QueryInfo
	(*ResPackBlock)(nil),  // 12: blockchaingrpc.ResPackBlock
	(*PackBlock)(nil),     // 13: blockchaingrpc.PackBlock
	(*ResQuery)(nil),      // 14: blockchaingrpc.ResQuery
}
var file_blockchain_proto_depIdxs = []int32{
	7,  // 0: blockchaingrpc.RequestBody.Tx:type_name -> blockchaingrpc.Transaction
	3,  // 1: blockchaingrpc.Nodes.nodes:type_name -> blockchaingrpc.NodeInfo
	7,  // 2: blockchaingrpc.Block.TxInfos:type_name -> blockchaingrpc.Transaction
	8,  // 3: blockchaingrpc.ResBlocks.blocks:type_name -> blockchaingrpc.Block
	8,  // 4: blockchaingrpc.BlockChainService.DistributeBlock:input_type -> blockchaingrpc.Block
	9,  // 5: blockchaingrpc.BlockChainService.BlockSynchronization:input_type -> blockchaingrpc.ReqBlock
	9,  // 6: blockchaingrpc.BlockChainService.GetLatestBlock:input_type -> blockchaingrpc.ReqBlock
	3,  // 7: blockchaingrpc.BlockChainService.JoinGroup:input_type -> blockchaingrpc.NodeInfo
	4,  // 8: blockchaingrpc.BlockChainService.Request:input_type -> blockchaingrpc.RequestBody
	3,  // 9: blockchaingrpc.BlockChainService.QuorumHeartbeat:input_type -> blockchaingrpc.NodeInfo
	2,  // 10: blockchaingrpc.BlockChainService.GetAccountant:input_type -> blockchaingrpc.Heartbeat
	0,  // 11: blockchaingrpc.BlockChainService.GetShareChan:input_type -> blockchaingrpc.ShareChanName
	6,  // 12: blockchaingrpc.BlockChainService.DistributeBlock:output_type -> blockchaingrpc.VerifyInfo
	10, // 13: blockchaingrpc.BlockChainService.BlockSynchronization:output_type -> blockchaingrpc.ResBlocks
	8,  // 14: blockchaingrpc.BlockChainService.GetLatestBlock:output_type -> blockchaingrpc.Block
	5,  // 15: blockchaingrpc.BlockChainService.JoinGroup:output_type -> blockchaingrpc.Nodes
	6,  // 16: blockchaingrpc.BlockChainService.Request:output_type -> blockchaingrpc.VerifyInfo
	2,  // 17: blockchaingrpc.BlockChainService.QuorumHeartbeat:output_type -> blockchaingrpc.Heartbeat
	6,  // 18: blockchaingrpc.BlockChainService.GetAccountant:output_type -> blockchaingrpc.VerifyInfo
	1,  // 19: blockchaingrpc.BlockChainService.GetShareChan:output_type -> blockchaingrpc.ShareChanBody
	12, // [12:20] is the sub-list for method output_type
	4,  // [4:12] is the sub-list for method input_type
	4,  // [4:4] is the sub-list for extension type_name
	4,  // [4:4] is the sub-list for extension extendee
	0,  // [0:4] is the sub-list for field type_name
}

func init() { file_blockchain_proto_init() }
func file_blockchain_proto_init() {
	if File_blockchain_proto != nil {
		return
	}
	if !protoimpl.UnsafeEnabled {
		file_blockchain_proto_msgTypes[0].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ShareChanName); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_blockchain_proto_msgTypes[1].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ShareChanBody); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_blockchain_proto_msgTypes[2].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Heartbeat); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_blockchain_proto_msgTypes[3].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*NodeInfo); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_blockchain_proto_msgTypes[4].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*RequestBody); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_blockchain_proto_msgTypes[5].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Nodes); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_blockchain_proto_msgTypes[6].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*VerifyInfo); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_blockchain_proto_msgTypes[7].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Transaction); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_blockchain_proto_msgTypes[8].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Block); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_blockchain_proto_msgTypes[9].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ReqBlock); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_blockchain_proto_msgTypes[10].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ResBlocks); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_blockchain_proto_msgTypes[11].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*QueryInfo); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_blockchain_proto_msgTypes[12].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ResPackBlock); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_blockchain_proto_msgTypes[13].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*PackBlock); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_blockchain_proto_msgTypes[14].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ResQuery); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_blockchain_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   15,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_blockchain_proto_goTypes,
		DependencyIndexes: file_blockchain_proto_depIdxs,
		MessageInfos:      file_blockchain_proto_msgTypes,
	}.Build()
	File_blockchain_proto = out.File
	file_blockchain_proto_rawDesc = nil
	file_blockchain_proto_goTypes = nil
	file_blockchain_proto_depIdxs = nil
}

// Reference imports to suppress errors if they are not otherwise used.
var _ context.Context
var _ grpc.ClientConnInterface

// This is a compile-time assertion to ensure that this generated file
// is compatible with the grpc package it is being compiled against.
const _ = grpc.SupportPackageIsVersion6

// BlockChainServiceClient is the client API for BlockChainService service.
//
// For semantics around ctx use and closing/ending streaming RPCs, please refer to https://godoc.org/google.golang.org/grpc#ClientConn.NewStream.
type BlockChainServiceClient interface {
	/// 分发区块
	DistributeBlock(ctx context.Context, in *Block, opts ...grpc.CallOption) (*VerifyInfo, error)
	// 区块同步
	BlockSynchronization(ctx context.Context, in *ReqBlock, opts ...grpc.CallOption) (*ResBlocks, error)
	// 获取某个节点最新区块信息
	GetLatestBlock(ctx context.Context, in *ReqBlock, opts ...grpc.CallOption) (*Block, error)
	// 加入集群
	JoinGroup(ctx context.Context, in *NodeInfo, opts ...grpc.CallOption) (*Nodes, error)
	Request(ctx context.Context, in *RequestBody, opts ...grpc.CallOption) (*VerifyInfo, error)
	QuorumHeartbeat(ctx context.Context, in *NodeInfo, opts ...grpc.CallOption) (*Heartbeat, error)
	GetAccountant(ctx context.Context, in *Heartbeat, opts ...grpc.CallOption) (*VerifyInfo, error)
	GetShareChan(ctx context.Context, in *ShareChanName, opts ...grpc.CallOption) (*ShareChanBody, error)
}

type blockChainServiceClient struct {
	cc grpc.ClientConnInterface
}

func NewBlockChainServiceClient(cc grpc.ClientConnInterface) BlockChainServiceClient {
	return &blockChainServiceClient{cc}
}

func (c *blockChainServiceClient) DistributeBlock(ctx context.Context, in *Block, opts ...grpc.CallOption) (*VerifyInfo, error) {
	out := new(VerifyInfo)
	err := c.cc.Invoke(ctx, "/blockchaingrpc.BlockChainService/DistributeBlock", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

func (c *blockChainServiceClient) BlockSynchronization(ctx context.Context, in *ReqBlock, opts ...grpc.CallOption) (*ResBlocks, error) {
	out := new(ResBlocks)
	err := c.cc.Invoke(ctx, "/blockchaingrpc.BlockChainService/BlockSynchronization", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

func (c *blockChainServiceClient) GetLatestBlock(ctx context.Context, in *ReqBlock, opts ...grpc.CallOption) (*Block, error) {
	out := new(Block)
	err := c.cc.Invoke(ctx, "/blockchaingrpc.BlockChainService/GetLatestBlock", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

func (c *blockChainServiceClient) JoinGroup(ctx context.Context, in *NodeInfo, opts ...grpc.CallOption) (*Nodes, error) {
	out := new(Nodes)
	err := c.cc.Invoke(ctx, "/blockchaingrpc.BlockChainService/JoinGroup", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

func (c *blockChainServiceClient) Request(ctx context.Context, in *RequestBody, opts ...grpc.CallOption) (*VerifyInfo, error) {
	out := new(VerifyInfo)
	err := c.cc.Invoke(ctx, "/blockchaingrpc.BlockChainService/Request", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

func (c *blockChainServiceClient) QuorumHeartbeat(ctx context.Context, in *NodeInfo, opts ...grpc.CallOption) (*Heartbeat, error) {
	out := new(Heartbeat)
	err := c.cc.Invoke(ctx, "/blockchaingrpc.BlockChainService/QuorumHeartbeat", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

func (c *blockChainServiceClient) GetAccountant(ctx context.Context, in *Heartbeat, opts ...grpc.CallOption) (*VerifyInfo, error) {
	out := new(VerifyInfo)
	err := c.cc.Invoke(ctx, "/blockchaingrpc.BlockChainService/GetAccountant", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

func (c *blockChainServiceClient) GetShareChan(ctx context.Context, in *ShareChanName, opts ...grpc.CallOption) (*ShareChanBody, error) {
	out := new(ShareChanBody)
	err := c.cc.Invoke(ctx, "/blockchaingrpc.BlockChainService/GetShareChan", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

// BlockChainServiceServer is the server API for BlockChainService service.
type BlockChainServiceServer interface {
	/// 分发区块
	DistributeBlock(context.Context, *Block) (*VerifyInfo, error)
	// 区块同步
	BlockSynchronization(context.Context, *ReqBlock) (*ResBlocks, error)
	// 获取某个节点最新区块信息
	GetLatestBlock(context.Context, *ReqBlock) (*Block, error)
	// 加入集群
	JoinGroup(context.Context, *NodeInfo) (*Nodes, error)
	Request(context.Context, *RequestBody) (*VerifyInfo, error)
	QuorumHeartbeat(context.Context, *NodeInfo) (*Heartbeat, error)
	GetAccountant(context.Context, *Heartbeat) (*VerifyInfo, error)
	GetShareChan(context.Context, *ShareChanName) (*ShareChanBody, error)
}

// UnimplementedBlockChainServiceServer can be embedded to have forward compatible implementations.
type UnimplementedBlockChainServiceServer struct {
}

func (*UnimplementedBlockChainServiceServer) DistributeBlock(context.Context, *Block) (*VerifyInfo, error) {
	return nil, status.Errorf(codes.Unimplemented, "method DistributeBlock not implemented")
}
func (*UnimplementedBlockChainServiceServer) BlockSynchronization(context.Context, *ReqBlock) (*ResBlocks, error) {
	return nil, status.Errorf(codes.Unimplemented, "method BlockSynchronization not implemented")
}
func (*UnimplementedBlockChainServiceServer) GetLatestBlock(context.Context, *ReqBlock) (*Block, error) {
	return nil, status.Errorf(codes.Unimplemented, "method GetLatestBlock not implemented")
}
func (*UnimplementedBlockChainServiceServer) JoinGroup(context.Context, *NodeInfo) (*Nodes, error) {
	return nil, status.Errorf(codes.Unimplemented, "method JoinGroup not implemented")
}
func (*UnimplementedBlockChainServiceServer) Request(context.Context, *RequestBody) (*VerifyInfo, error) {
	return nil, status.Errorf(codes.Unimplemented, "method Request not implemented")
}
func (*UnimplementedBlockChainServiceServer) QuorumHeartbeat(context.Context, *NodeInfo) (*Heartbeat, error) {
	return nil, status.Errorf(codes.Unimplemented, "method QuorumHeartbeat not implemented")
}
func (*UnimplementedBlockChainServiceServer) GetAccountant(context.Context, *Heartbeat) (*VerifyInfo, error) {
	return nil, status.Errorf(codes.Unimplemented, "method GetAccountant not implemented")
}
func (*UnimplementedBlockChainServiceServer) GetShareChan(context.Context, *ShareChanName) (*ShareChanBody, error) {
	return nil, status.Errorf(codes.Unimplemented, "method GetShareChan not implemented")
}

func RegisterBlockChainServiceServer(s *grpc.Server, srv BlockChainServiceServer) {
	s.RegisterService(&_BlockChainService_serviceDesc, srv)
}

func _BlockChainService_DistributeBlock_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(Block)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(BlockChainServiceServer).DistributeBlock(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/blockchaingrpc.BlockChainService/DistributeBlock",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(BlockChainServiceServer).DistributeBlock(ctx, req.(*Block))
	}
	return interceptor(ctx, in, info, handler)
}

func _BlockChainService_BlockSynchronization_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(ReqBlock)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(BlockChainServiceServer).BlockSynchronization(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/blockchaingrpc.BlockChainService/BlockSynchronization",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(BlockChainServiceServer).BlockSynchronization(ctx, req.(*ReqBlock))
	}
	return interceptor(ctx, in, info, handler)
}

func _BlockChainService_GetLatestBlock_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(ReqBlock)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(BlockChainServiceServer).GetLatestBlock(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/blockchaingrpc.BlockChainService/GetLatestBlock",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(BlockChainServiceServer).GetLatestBlock(ctx, req.(*ReqBlock))
	}
	return interceptor(ctx, in, info, handler)
}

func _BlockChainService_JoinGroup_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(NodeInfo)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(BlockChainServiceServer).JoinGroup(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/blockchaingrpc.BlockChainService/JoinGroup",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(BlockChainServiceServer).JoinGroup(ctx, req.(*NodeInfo))
	}
	return interceptor(ctx, in, info, handler)
}

func _BlockChainService_Request_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(RequestBody)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(BlockChainServiceServer).Request(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/blockchaingrpc.BlockChainService/Request",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(BlockChainServiceServer).Request(ctx, req.(*RequestBody))
	}
	return interceptor(ctx, in, info, handler)
}

func _BlockChainService_QuorumHeartbeat_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(NodeInfo)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(BlockChainServiceServer).QuorumHeartbeat(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/blockchaingrpc.BlockChainService/QuorumHeartbeat",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(BlockChainServiceServer).QuorumHeartbeat(ctx, req.(*NodeInfo))
	}
	return interceptor(ctx, in, info, handler)
}

func _BlockChainService_GetAccountant_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(Heartbeat)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(BlockChainServiceServer).GetAccountant(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/blockchaingrpc.BlockChainService/GetAccountant",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(BlockChainServiceServer).GetAccountant(ctx, req.(*Heartbeat))
	}
	return interceptor(ctx, in, info, handler)
}

func _BlockChainService_GetShareChan_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(ShareChanName)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(BlockChainServiceServer).GetShareChan(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/blockchaingrpc.BlockChainService/GetShareChan",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(BlockChainServiceServer).GetShareChan(ctx, req.(*ShareChanName))
	}
	return interceptor(ctx, in, info, handler)
}

var _BlockChainService_serviceDesc = grpc.ServiceDesc{
	ServiceName: "blockchaingrpc.BlockChainService",
	HandlerType: (*BlockChainServiceServer)(nil),
	Methods: []grpc.MethodDesc{
		{
			MethodName: "DistributeBlock",
			Handler:    _BlockChainService_DistributeBlock_Handler,
		},
		{
			MethodName: "BlockSynchronization",
			Handler:    _BlockChainService_BlockSynchronization_Handler,
		},
		{
			MethodName: "GetLatestBlock",
			Handler:    _BlockChainService_GetLatestBlock_Handler,
		},
		{
			MethodName: "JoinGroup",
			Handler:    _BlockChainService_JoinGroup_Handler,
		},
		{
			MethodName: "Request",
			Handler:    _BlockChainService_Request_Handler,
		},
		{
			MethodName: "QuorumHeartbeat",
			Handler:    _BlockChainService_QuorumHeartbeat_Handler,
		},
		{
			MethodName: "GetAccountant",
			Handler:    _BlockChainService_GetAccountant_Handler,
		},
		{
			MethodName: "GetShareChan",
			Handler:    _BlockChainService_GetShareChan_Handler,
		},
	},
	Streams:  []grpc.StreamDesc{},
	Metadata: "blockchain.proto",
}
